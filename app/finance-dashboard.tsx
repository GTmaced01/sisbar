...
function SuppliersSection({ suppliers, onNew, onEdit }: { suppliers: Supplier[]; onNew: () => void; onEdit: (supplier: Supplier) => void }) {
  return <><div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h3 className="text-xl font-semibold">Fornecedores</h3><p className="mt-1 text-sm text-slate-500">Centralize contatos usados nas compras e despesas.</p></div><Button className="bg-[#ef7d22] hover:bg-[#d86d18]" onClick={onNew}><Plus /> Novo fornecedor</Button></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{suppliers.map((supplier) => <Card key={supplier.id} className="gap-0 border-slate-200 py-0 shadow-none"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><span className="grid size-10 place-items-center rounded-xl bg-slate-100 text-[#102a43]"><Building2 className="size-5" /></span><Badge variant="secondary" className={supplier.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}>{supplier.active ? "Ativo" : "Inativo"}</Badge></div><h4 className="mt-4 font-semibold">{supplier.name}</h4><p className="mt-1 text-xs text-slate-500">{supplier.tax_id || supplier.phone || supplier.email || "Sem dados adicionais"}</p><Button variant="outline" size="sm" className="mt-4 w-full" onClick={() => onEdit(supplier)}><Pencil /> Editar fornecedor</Button></CardContent></Card>)}{!suppliers.length && <Card className="border-dashed shadow-none md:col-span-2 xl:col-span-3"><CardContent className="p-10 text-center text-sm text-slate-500">Nenhum fornecedor cadastrado.</CardContent></Card>}</div></>;
}
