...
function ReceivablesSection({ data, onPay, company }: { data: ReceivablesData | null; onPay: (employee: Employee) => void; company: Company }) {
  ...
  return <><SectionTitle eyebrow="Financeiro" title="Contas a receber" description="Saldos em aberto agrupados por funcionário." /><Card className="mb-5 gap-0 border-0 bg-[#102a43] py-0 text-white shadow-none"><CardContent className="flex flex-col justify-between gap-3 p-5 sm:flex-row sm:items-center sm:p-6"><div><p className="text-sm text-slate-300">Total em aberto</p><p className="mt-1 text-3xl font-bold">{brl.format(data?.total ?? 0)}</p></div><div className="flex items-center gap-2 text-sm text-slate-300"><Users className="size-4" /> {data?.receivables.length ?? 0} pessoas com saldo</div></CardContent></Card><div className="space-y-3">{data?.receivables.length ? data.receivables.map((employee) => <Card key={employee.id} className="min-w-0 gap-0 border-slate-200 py-0 shadow-none"><CardContent className="p-4 sm:flex sm:items-center sm:gap-4 sm:p-5"><div className="flex min-w-0 items-center gap-3 sm:flex-1"><div className="grid size-10 shrink-0 place-items-center rounded-full bg-slate-100 font-semibold text-[#102a43]">{employee.full_name.slice(0, 1)}</div><div className="min-w-0"><p className="truncate font-semibold">{employee.full_name}</p><p className="mt-0.5 text-xs leading-5 text-slate-500">{employee.department_name} · Matrícula {employee.enrollment} · {employee.open_sales} {employee.open_sales === 1 ? "venda" : "vendas"}</p></div></div><div className="mt-4 flex items-end justify-between gap-3 sm:mt-0 sm:block sm:text-right"><div><p className="text-xs text-slate-500">Saldo</p><p className="text-xl font-bold">{brl.format(employee.balance)}</p></div></div><div className="mt-4 grid grid-cols-2 gap-2 sm:mt-0 sm:flex"><Button variant="outline" size="sm" className="w-full" onClick={() => whatsapp(employee)}><MessageCircle /> Cobrar</Button><Button size="sm" className="w-full bg-[#102a43] hover:bg-[#173d5f]" onClick={() => onPay(employee)}><Banknote /> Dar baixa</Button></div></CardContent></Card>) : <EmptyState icon={CheckCircle2} title="Tudo em dia" text="Não há contas a receber no momento." />}</div></>;
}
...
function ReportSection({ report, onReport, session }: { report: ReportData | null; onReport: (report: ReportData) => void; session: Session }) {
  ...
  return <><SectionTitle eyebrow="Fechamento" title="Relatório mensal" description="Consolide vendas, recebimentos, produtos e funcionários." action={<div className="grid w-full gap-2 sm:flex sm:w-auto"><Input type="month" className="w-full bg-white sm:w-40" value={month} onChange={(event) => setMonth(event.target.value)} /><Button className="w-full bg-[#102a43] hover:bg-[#173d5f] sm:w-auto" onClick={() => void generate()} disabled={busy}>{busy ? "Gerando..." : "Gerar relatório"}</Button></div>} />{report ? <><div className="mb-5 flex justify-stretch sm:justify-end"><Button variant="outline" className="w-full sm:w-auto" onClick={download}><Download /> Exportar CSV</Button></div><div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4"><MetricCard label="Total vendido" value={brl.format(report.summary.total_sold)} icon={TrendingUp} /><MetricCard label="Recebido no mês" value={brl.format(report.summary.received_in_month)} icon={ArrowDownRight} /><MetricCard label="Em aberto" value={brl.format(report.summary.outstanding_from_month)} icon={ArrowUpRight} /><MetricCard label="Itens retirados" value={String(report.summary.items_count)} icon={Boxes} /></div><div className="mt-5 grid min-w-0 gap-5 xl:mt-6 xl:grid-cols-2 xl:gap-6"><Card className="min-w-0 gap-0 overflow-hidden border-slate-200 py-0 shadow-none"><CardHeader className="p-4 sm:p-6"><CardTitle>Por funcionário</CardTitle></CardHeader><CardContent className="p-0"><Table className="min-w-[440px]"><TableHeader><TableRow><TableHead>Funcionário</TableHead><TableHead className="text-right">Comprado</TableHead><TableHead className="text-right">Saldo</TableHead></TableRow></TableHeader><TableBody>{report.by_employee.map((row) => <TableRow key={row.enrollment}><TableCell><p className="font-medium">{row.name}</p><p className="text-xs text-slate-500">{row.enrollment}</p></TableCell><TableCell className="text-right">{brl.format(row.total)}</TableCell><TableCell className="text-right font-medium">{brl.format(row.balance)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card><Card className="min-w-0 gap-0 overflow-hidden border-slate-200 py-0 shadow-none"><CardHeader className="p-4 sm:p-6"><CardTitle>Produtos mais retirados</CardTitle></CardHeader><CardContent className="p-0"><Table className="min-w-[400px]"><TableHeader><TableRow><TableHead>Produto</TableHead><TableHead className="text-right">Qtd.</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader><TableBody>{report.by_product.map((row) => <TableRow key={row.name}><TableCell className="font-medium">{row.name}</TableCell><TableCell className="text-right">{row.quantity}</TableCell><TableCell className="text-right">{brl.format(row.total)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card></div></> : <EmptyState icon={FileBarChart} title="Selecione o mês" text="Gere o relatório para consultar e exportar os dados." />}</>;
}
        ...
        <form className="space-y-4" onSubmit={save}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col items-stretch gap-3 rounded-lg border border-slate-200 p-3 sm:flex-row sm:items-center">
              ...
              {imagePreview && <Button type="button" variant="ghost" size="sm" className="w-full text-rose-700 sm:w-auto" onClick={() => { setImagePreview(null); setRemoveImage(true); }}>Remover</Button>}
            </div>
          </div>
